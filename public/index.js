// public/index.js - full version with all provided hoc10 links (no truncation)
// - populateSubjects: build lists of subjects per grade
// - booksMap: SGK / SBT hoc10 links (all links provided by user are included)
// - delegation and showSubjectDetail render book links as anchors (open in new tab)
// - exports window.FP for debugging
(function(){
  'use strict';
  window.FP = window.FP || {};

  const log = (...args) => { try{ console.info('[FP]', ...args); }catch(e){} };
  const dbg = (...args) => { try{ console.debug('[FP-debug]', ...args); }catch(e){} };

  function hoc10Search(subject, grade){
    const q = encodeURIComponent((subject||'') + (grade ? ' ' + grade : ''));
  ...
  // booksMap: full insertion of the links you listed
  const booksMap = {
    /* NGỮ VĂN */
    '6|Ngữ văn': [
  ...
  // robust lookup: try exact grade|subject, subject-only, and suffix matches
  function lookupBooks(grade, subject){
    const sub = (subject || '').trim();
    const gradeRaw = (grade === undefined || grade === null) ? '' : String(grade).trim();
    const gradeNorm = gradeRaw.replace(/^Lớp\s*/i, '').trim();

    const candidates = [];
    if(gradeNorm) candidates.push(`${gradeNorm}|${sub}`);
    if(gradeRaw) candidates.push(`${gradeRaw}|${sub}`);
    candidates.push(`${sub}`);
    // also look for any key where last segment equals subject
    Object.keys(booksMap).forEach(k => {
      const parts = k.split('|');
      const kSubject = parts[parts.length-1];
      if(kSubject === sub && !candidates.includes(k)) candidates.push(k);
    });

    dbg('lookupBooks candidates', candidates);
    for(const k of candidates){
      if(booksMap.hasOwnProperty(k) && Array.isArray(booksMap[k]) && booksMap[k].length){
        dbg('lookupBooks found for key', k);
        return booksMap[k];
      }
    }
    dbg('lookupBooks none found for', grade, subject);
    return [];
  }

  function renderBookLinks(grade, subject){
    const list = lookupBooks(grade, subject) || [];
    ...
    container.addEventListener('click', function(e){
      ...
      while(el && el !== container){
        if(el.matches && el.matches('a.subject')){
          ...
          const grade = el.dataset.grade || (el.closest && el.closest('[id^="grade-"]') ? el.closest('[id^="grade-"]').id.replace('grade-','') : '');
  ...
  function ensureAuthNavigation(){
    ['link-login','link-register','link-upload','btn-upload-quick'].forEach(id=>{
  ...
  window.FP.lookupBooks = lookupBooks;
